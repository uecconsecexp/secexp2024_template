...
本演習ではbashを使用するため、ログインシェルを`bash`に確実に変更してください。

変更ページ(学内限定): https://www.cc.uec.ac.jp/in/ch_shell/

**必ず変更してください**。変更しないと2回目以降でまた古いバージョンのGoが参照されてしまいます。

ログインシェル変更には再起動が必要になりますが、とりあえず`exec bash`で`bash`を起動できるため再起動の代わりとして初回はそうしてもよいです。
